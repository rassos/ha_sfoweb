...
class SFOScraper:
    ...
    async def _verify_authentication(self, session: aiohttp.ClientSession) -> bool:
        """Verify if authentication was successful by following redirects."""
        try:
            # After login, we need to follow the SAML redirect chain
            _LOGGER.info("Following SAML redirect chain after authentication...")
            
            # The current response might contain auto-redirect forms or meta refreshes
            # Let's try to get to the actual appointments page step by step
            
            # First try: Direct access to appointments URL
            async with session.get(APPOINTMENTS_URL) as response:
                _LOGGER.info(f"Direct appointments access: {response.status}")
                if response.status == 200:
                    html = await response.text()
                    _LOGGER.debug(f"Appointments page length: {len(html)}")
                    
                    # Check if this looks like the real appointments page
                    html_lower = html.lower()
                    
                    # Bad indicators - still on login/selection page
                    bad_indicators = [
                        'forældre log-in',
                        'vælg login metode',
                        'du kan her logge ind',
                        'login metode herunder',
                        'nemlog-in'
                    ]
                    
                    is_login_page = any(indicator in html_lower for indicator in bad_indicators)
                    
                    if is_login_page:
                        _LOGGER.warning("Still on login selection page after authentication")
                        return False
                    
                    # Good indicators - actual content page
                    good_indicators = [
                    ...
                    has_content = any(indicator in html_lower for indicator in good_indicators)
                    
                    if has_content:
                        _LOGGER.info("Authentication successful - found content page")
                        return True
                    
                    # If it contains 'aftale' but not other content, might need navigation
                    if 'aftale' in html_lower:
                        _LOGGER.info("Found 'aftale' - authentication likely successful")
                        return True
    ...
    async def _fetch_appointments_data(self, session: aiohttp.ClientSession) -> List[Dict[str, Any]]:
        ...
        try:
            _LOGGER.info("Attempting to access appointments page...")
            
            # Try different potential URLs for appointments
            appointment_urls = [
                APPOINTMENTS_URL,
                "https://soestjernen.sfoweb.dk/aftaler",
                "https://soestjernen.sfoweb.dk/foraeldre/aftaler",
                "https://soestjernen.sfoweb.dk/parent/appointments",
                "https://soestjernen.sfoweb.dk/kalender",
                "https://soestjernen.sfoweb.dk/dashboard",
                "https://soestjernen.sfoweb.dk/home",
                "https://soestjernen.sfoweb.dk/main",
            ]
            ...
            for url in appointment_urls:
                ...
                try:
                    async with session.get(url) as response:
                        ...
                        if response.status == 200:
                            html = await response.text()
                            _LOGGER.info(f"Page length: {len(html)}")
                            ...
                            is_login_page = any(indicator in html_lower for indicator in login_indicators)
                            
                            if is_login_page:
                                _LOGGER.warning(f"URL {url} returned login page")
                                continue
                                
                            # Try to parse appointments from this page
                            page_appointments = self._parse_appointments_html(html)
                            
                            if page_appointments:
                                _LOGGER.info(f"Found {len(page_appointments)} appointments at {url}")
                                appointments.extend(page_appointments)
                                return appointments
                            
                            # Check if page has useful navigation links
                            nav_links = self._extract_navigation_links(html, str(response.url))
                            if nav_links:
                                _LOGGER.info(f"Found {len(nav_links)} navigation links")
                                
                                # Try following navigation links
                                for nav_url in nav_links[:3]:  # Limit to first 3
                                    try:
                                        _LOGGER.info(f"Following navigation link: {nav_url}")
                                        async with session.get(nav_url) as nav_response:
                                            if nav_response.status == 200:
                                                nav_html = await nav_response.text()
                                                nav_appointments = self._parse_appointments_html(nav_html)
                                                if nav_appointments:
                                                    _LOGGER.info(f"Found {len(nav_appointments)} appointments via navigation")
                                                    appointments.extend(nav_appointments)
                                                    return appointments
                                    except Exception as e:
                                        _LOGGER.debug(f"Navigation link {nav_url} failed: {e}")
                                
                            # If this page has some content but no appointments, it might be the right area
                            if any(term in html_lower for term in ['aftale', 'kalender', 'besked', 'profil']):
                                _LOGGER.info(f"Found content page at {url} but no appointments")
                                # Still try to parse in case there are appointments in a different format
                                appointments = self._parse_appointments_html(html)
                                break
                                
                except Exception as e:
                    _LOGGER.debug(f"Error accessing {url}: {e}")
                    continue
            
            if not appointments:
                _LOGGER.info("No appointments found - may be legitimately empty")
    ...
    def _extract_navigation_links(self, html: str, base_url: str) -> List[str]:
        """Extract potential navigation links from the page."""
        links = []
        ...
        try:
            ...
            # Look for links that might lead to appointments
            keywords = [
                'aftale', 'appointment', 'kalender', 'calendar',
                'tidsplan', 'schedule', 'booking', 'reservation'
            ]
            ...
            for link in soup.find_all('a', href=True):
                ...
                # Check if link text or href contains appointment-related keywords
            ...
            _LOGGER.debug(f"Error extracting navigation links: {e}")
